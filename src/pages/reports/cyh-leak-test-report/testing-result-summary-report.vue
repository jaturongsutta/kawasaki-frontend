<template>
  <div>
    <v-card>
      <v-card-title>
        <h4>Testing Result Summary Report</h4>
      </v-card-title>
      <v-card-text>
...
const headersDetail = [
  ...
  { title: "Total Test", key: "Total_Test", sortable: false },

  { title: "FG", key: "FG", sortable: false },

  // ---- NG CH1–CH5 (ตาม JSON: NG_P1, NG_P2...) ----
  { title: "P1, OH, CH1", key: "NG1", sortable: false },
  { title: "P2, WJ, CH2", key: "NG2", sortable: false },
  { title: "P3, CC, CH3", key: "NG3", sortable: false },
  { title: "P4, -, CH4", key: "NG4", sortable: false },
  { title: "P5, T/B, CH5", key: "NG5", sortable: false },
];
...
const loadData = async (paginate) => {
  ...
  try {
    ...
    const response = await api.searchCYHTestingResultSummary(data);
...
const onExport = async () => {
  try {
    ...
    const response = await api.exportCYHTestingResultSummaryReport(formSearch.value);
    ...
    link.download = `Report2-Testing-Result-Summary-${timestamp}.xlsx`;
