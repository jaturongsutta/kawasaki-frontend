<template>
  <div>
    <v-card>
      <v-card-title>
        <h4>Machine No Plan Summary Report</h4>
      </v-card-title>
      <v-card-text>
...
const headersDetail = [
  ...
  { title: "No Plan (Min.)", key: "Loss_mins", sortable: false },
...
const loadData = async (paginate) => {
  ...
  try {
    ...
    const response = await api.searchCYHMachineNoPlanSummary(data);
...
const onExport = async () => {
  try {
    ...
    const response = await api.exportCYHMachineNoPlanSummaryReport(formSearch.value);
    ...
    link.download = `Report5-Machine-No-Plan-Summary-${timestamp}.xlsx`;
